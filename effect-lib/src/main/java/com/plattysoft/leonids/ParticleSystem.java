...
public class ParticleSystem {
	...
	private static final long TIMMERTASK_INTERVAL = 40;
	...
	public ParticleSystem(ViewGroup parentView, int maxParticles, Drawable drawable, long timeToLive) {
		...
		if (drawable instanceof AnimationDrawable) {
			...
			int bitmapRondom = animation.getNumberOfFrames();
			Random random = new Random();
			for (int i = 0; i < mMaxParticles; i++) {
				mParticles.add(new Particle(((BitmapDrawable) animation.getFrame(random.nextInt(bitmapRondom))).getBitmap()));
			}
		} else {
			Bitmap bitmap = null;
	 ...
	 *
	 * @param v            The parent viewgroup
	 * @param maxParticles The maximum number of particles
	 * @param bitmaps      The bitmap to use as particle
	 * @param timeToLive   The time to live for the particles
	 */
	public ParticleSystem(ViewGroup v, int maxParticles, Bitmap[] bitmaps, long timeToLive) {
		this(v, maxParticles, timeToLive);
		for (int i = 0; i < mMaxParticles; i++) {
	 ...
	 * @param emiter             View from which center the particles will be emited
	 * @param particlesPerSecond Number of particles per second that will be emited (evenly distributed)
	 * @param emitingTime        time the emiter will be emiting particles,(oneshot)
	 */
	...
	public void updateEmitPoint(View emiter) {
		updateEmitPoint(emiter, Gravity.CENTER);
	}
