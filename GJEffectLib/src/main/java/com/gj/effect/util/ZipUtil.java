...
public class ZipUtil {
	...
	/**
	 * DeCompress the ZIP to the path, no update if file exists.
	 *
...
//				file.setReadOnly();
	...
	public static List<File> GetFileList(String zipFileString, boolean bContainFolder, boolean bContainFile) throws Exception {
		...
		while ((zipEntry = inZip.getNextEntry()) != null) {
			...
			if (zipEntry.isDirectory()) {
				...
				Log.e("xxx", "file path" + file.getPath() + "," + file.getAbsolutePath());
				if (bContainFile) {
					fileList.add(file);
				}
			}
	...
	/**
	 * return file for the specified filename in the ZIP
	 *
	 * @param zipFileString ZIP name
	 ...
	 * @throws Exception
	 */
	public static File GetFile(String zipFileString, String fileName) throws Exception {
		ZipInputStream inZip = new ZipInputStream(new FileInputStream(zipFileString));
		ZipEntry zipEntry;
		String szName = "";
		while ((zipEntry = inZip.getNextEntry()) != null) {
			szName = zipEntry.getName();
			if (szName.endsWith(fileName)) {
				File file = new File(szName);
				return file;
